...
DEFAULTS = {
    ...
    # Каким голосом CNPS озвучивает ответы модулей и болталку — поле
    # "voice" в заявке (см. ../cnps/cnps_design.md, «Манифест заявки»).
    # "gtts" — женский Google TTS (голос ассистента), отдельный от
    # мужского piper, которым CNPS говорит системные сообщения.
    # "" — не проставлять (CNPS решит по source: outputstructurizer -> gtts).
    "playback_voice": "gtts",
    ...
    "paths": {
        ...
        # queue_dir.
        "stale_subdir": "stale",
        # Выход: папка-очередь заявок на воспроизведение. outputstructurizer
        # кладёт сюда по одному <task_id>.json на ответ, CNPS его
        # озвучивает. Относительно папки со скриптом. cnps_tasks_dir —
        # основной путь (system/cnps/); audio_tasks_dir — откат на
        # старую заглушку system/audio/, если CNPS ещё не собран.
        "cnps_tasks_dir": "../cnps/tasks",
