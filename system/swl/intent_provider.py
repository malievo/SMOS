...
SYSTEM_PROMPT = (
    ...
    "у каждой: goal — ключ цели, description — что это, needs — какие данные "
    "нужны для неё внутри системы, params — СЛОТЫ, которые надо заполнить из "
    "фразы, вида имя -> {type, values?, required?}), и фразу пользователя.\n"
    "1. Определи, какую ОДНУ цель из списка просит пользователь.\n"
    "2. Заполни params этой цели значениями из фразы:\n"
    "   - бери значение слота так, как оно сказано во фразе (время, текст "
    "заметки, выражение и т.п.); приводить к формату НЕ надо — это сделает "
    "система;\n"
    "   - для слота с type enum/crud_action/toggle выбери значение ИЗ его "
    "values;\n"
    "   - чего во фразе нет — НЕ придумывай, просто не клади этот ключ в "
    "params.\n"
    "Ответь СТРОГО одним JSON-объектом, без markdown и без пояснений:\n"
    '{"goal": "<ключ из списка>", "params": {"<слот>": <значение>, ...}}\n'
