...
DEFAULTS = {
    ...
    "mute": {
        # Глушение микрофона на время воспроизведения. Пока audio.py
        # что-то произносит, он кладёт этот файл — система распознавания
        # (wake.py) видит его и на это время перестаёт слушать, чтобы не
        # услышать саму себя и не закольцеваться (ответ -> услышали ->
        # команда -> снова ответ). Контракт файла — см. wake.py, функция
        # mic_muted. Путь — относительно корня проекта (папка с smos.root),
        # чтобы указывал в чужую подсистему (rvs) явно и переносимо.
        "flag_path": "system/fwl/rvs/flags/mute.flag",
        # Запас поверх timeout движка синтеза, сек. audio.py пишет флаг в
        # режиме "until" (заглушить до момента X), X = сейчас + предельная
        # длительность одной фразы (max timeout движков) + этот запас.
        # Если демон упадёт, не сняв флаг, микрофон оглохнет максимум на
        # это время и сам отпустит — чинить вручную не надо.
        "expiry_margin_sec": 2,
    },
...
def project_root(base_dir: Path) -> Path | None:
    """Корень проекта SMOS (папка с файлом-маркером smos.root) или None,
    если не найден. Нужен тем местам audio.py, что обращаются к файлам
    вне своей подсистемы (например, флаг глушения микрофона в rvs/)."""
